function [bestSolution, bestFitness, iteration]=bsafdbpopdinamik60(fhd, dimension, maxIteration, fNumber)
...
fe=0;
    while fe<maxIteration
        ...
        for i=1:popsize
            if rand < 0.6
                if FddbPopDinamik(pop, fitnesspop, i, offsprings(i,:), fitnessoffsprings(i), maxIteration, fe)
                    fitnesspop(i)=fitnessoffsprings(i);
                    pop(i,:)=offsprings(i,:);
                end
            else
                if fitnessoffsprings(i)<fitnesspop(i)
                    fitnesspop(i)=fitnessoffsprings(i);
                    pop(i,:)=offsprings(i,:);
                end
            end
